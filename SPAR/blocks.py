...
class ResidualBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if att_name.lower() == 'spar':
            ...
            raise Exception(
                "Attention type {} not implemented".format(att_name))
...
class HourGlassBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.depth:
            ...
            self.out_block = nn.Sequential(
                ConvLayer(self.c_mid,
                          self.c_out,
                          norm_type='none',
                          relu_type='none'), nn.Sigmoid())
    ...
    def _generate_network(self, level):
        ...
        self.add_module('b2_' + str(level),
                        ConvLayer(c1, c2, scale='down', **self.kwargs))
        if level > 1:
